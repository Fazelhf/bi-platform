...
BOARDS: dict[str, dict] = {
    ...
    "commercial_foreign": {
        "title": "داشبورد بازرگانی خارجی",
        "subtitle": "پرونده‌ها بر اساس تاریخ ثبت سفارش، محموله‌ها بر اساس تاریخ حرکت",
        "widgets": [
            kpi("پرونده‌های باز", "foreign_orders", "orders", 0, 0,
                time={"mode": "all"},
                filters=[{"dim": "status", "op": "ne", "value": "closed"}]),
            kpi("مبلغ ارزی در جریان", "foreign_orders", "amount_fx", 3, 0,
                time={"mode": "all"},
                filters=[{"dim": "status", "op": "ne", "value": "closed"}]),
            kpi("محموله‌های در راه", "foreign_shipments", "shipments", 6, 0,
                time={"mode": "all"},
                filters=[{"dim": "status", "op": "ne", "value": "delivered"}]),
            kpi("وزن واردشده", "foreign_shipments", "weight_ton", 9, 0,
                time={"mode": "all"}),
            chart("hbar", "پرونده‌ها در هر مرحله", "foreign_orders", ["orders"],
                  "status", 0, 3, 6, 6, time={"mode": "all"}),
            chart("bar", "روند ثبت سفارش", "foreign_orders", ["orders"], "month",
                  6, 3, 6, 6, time={"mode": "last_n", "n": 12}),
            chart("donut", "ارزش پرونده‌ها به تفکیک کشور", "foreign_orders",
                  ["amount_fx"], "country", 0, 9, 4, 6, time={"mode": "all"}),
            chart("bar", "محموله‌ها و وزن", "foreign_shipments",
                  ["shipments", "weight_ton"], "month", 4, 9, 8, 6,
                  time={"mode": "last_n", "n": 12}),
            {
                "kind": "table", "title": "تامین‌کنندگان خارجی", "x": 0, "y": 15,
                "w": 12, "h": 6,
                "config": {
                    "dataset": "foreign_orders",
                    "metrics": ["orders", "amount_fx", "weight_ton"],
                    "dimension": "supplier", "time": {"mode": "all"}, "limit": 15,
                },
            },
        ],
    },
