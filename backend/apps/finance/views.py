...
class CashEntryView(APIView):
    ...
    def get(self, request):
        ...
        stored = {
            (m.period_id, m.direction, m.category_id): m
            for m in CashMovement.objects.filter(period_id__in=[d.id for d in days])
        }

        def cell(day, direction, category):
            m = stored.get((day.id, direction, category.id))
            return {
                "amount_rial": str(m.amount_rial) if m else "0",
                "credit_line": m.credit_line_id if m else None,
    ...
    def post(self, request):
        ...
        for day in request.data.get("days", []):
            ...
            for direction in (Direction.IN, Direction.OUT):
                for raw_category_id, cell in (day.get(direction) or {}).items():
                    amount = cell.get("amount_rial") if isinstance(cell, dict) else cell
                    line_id = cell.get("credit_line") if isinstance(cell, dict) else None
                    ...
                    existing = CashMovement.objects.filter(
                        ...
                        category_id=raw_category_id, credit_line_id=line_id,
                    ).first()
                    # Never stored and still zero: leave it alone rather than
                    # filling the table with empty days.
        ...
        return Response({"ok": True, "submitted": submit, "movements": written})
